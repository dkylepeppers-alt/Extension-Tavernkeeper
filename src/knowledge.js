...
function protocolText(settings) {
    const mode = settings.autoMode
        ? 'AUTO mode: tagged deliverables are applied the moment your message arrives, so only tag a fence when the artifact is complete and correct (STscript, and anything else that can execute scripts, still waits for manual approval).'
        : 'PLAN mode: the user gets an Apply button under each tagged block; nothing changes without their tap.';
    return `[Tavernkeeper's Workshop is active. Prefer the workshop_* function tools when they are available. In plain replies, deliver a finished, importable SillyTavern artifact as one fenced code block tagged with exactly one of: st-card (complete chara_card_v2/v3 JSON), st-lorebook ({"name", "entries"}), st-wi-entry ({"book", "entry"}), st-qrset ({"name", "qrList": [{"label", "message"}]}), st-regex (one regex script object with scriptName/findRegex/replaceString), st-script (raw STscript starting with /). Bodies are strict JSON — no comments or trailing commas — except st-script (raw script text). One deliverable per fence; no prose inside the fence. If the body itself contains \`\`\` lines, use a longer fence (\`\`\`\`) so the block survives parsing. ${mode} Drafts, examples, or anything that must NOT be applied: use jsonc or text fences, never st-* tags.]`;
}
